...
type
  ...
  TShapeType = (stLine, stSegmentLine, stPolyLine, stBezierCurve, stCircle,
  ...
  TLine = class(TShape)
    private
      _line: TGeneralLineEquation;
      _n: integer; //dolezite pri vytvarani
      _point: TPoint; //dolezite pri vytvarani
    protected
      function getCreated(): boolean; override;
      procedure changeZoom(a: Real); override;
    public
      ...
      //------resize-----
      procedure movePoint(i: Integer; p: TPoint); override;
      function removePoint(i: Integer): boolean; override; //v tomto pripade ma zmysel iba pri vytvarani
      procedure drawResizeControls(C: TCanvas); override;
      function getResizeControls(): TPoints; override;
      //------create-----
      ...
      constructor Create(); overload;
      constructor Create(a, b: TPoint; color: TColor = clBlack; width: Integer = 1); overload;
      constructor Create(x1, y1, x2, y2: Integer; color: TColor = clBlack; width: Integer = 1); overload;
      destructor Destroy; override;
  end;
...
function TLine.distance(a: TPoint): Integer;
begin
  Result := _line.pointDistance(a);
end;

function TLine.getCreated(): boolean;
begin
  Result := (_line <> nil) and (_n=2);
end;

procedure TLine.move(x, y: Integer);
begin
  _line.controlPoint.X := _line.controlPoint.X + x;
  _line.controlPoint.Y := _line.controlPoint.Y + y;
end;

procedure TLine.rotate(h: Real);
begin
  _line.rotate(h, rotatePoint);
end;

procedure TLine.resetRotatePoint(p: TPoint);
begin
  moveRotatePointTo(p)
end;

function TLine.saveToBuffer(var buff: TBuffer): Integer;
begin
  Result := 0;
end;

constructor TLine.loadFromBuffer(var p: TBuffer; count: Integer);
begin
end;

procedure TLine.drawResizeControls(C: TCanvas);
begin
  if (_line <> nil) then
  begin
    drawMoveControls(C);
    C.Pen.Style := psSolid;
    C.Rectangle(_line.controlPoint.point.X - 5,
                _line.controlPoint.point.Y - 5,
                _line.controlPoint.point.X + 5,
                _line.controlPoint.point.Y + 5);
  end;
end;

function TLine.getResizeControls(): TPoints;
begin
  SetLength(Result, 1);
  Result[0] := _line.controlPoint.point;
end;

procedure TLine.draw(C: TCanvas);
begin
  if (_line <> nil) then
  begin
    C.Pen.Mode := pmCopy;
    ...
    C.Pen.Width := width;
    C.Pen.Style := psSolid;
    C.MoveTo(_line.controlPoint.point.x + round(_line.controlVector.y * 100000),
             _line.controlPoint.point.y - round(_line.controlVector.x * 100000));
    C.LineTo(_line.controlPoint.point.x - round(_line.controlVector.y * 100000),
             _line.controlPoint.point.y + round(_line.controlVector.x * 100000));
  end;
end;

function TLine.getType(): TShapeType;
begin
  Result := stLine;
end;

procedure TLine.drawRotateControls(C: TCanvas);
begin
  if (_line <> nil) then
  begin
    drawMoveControls(C);
    ...
    //riadacia ciara
    C.Pen.Style := psDot;
    C.MoveTo(round(rotatePoint.x + (_line.controlVector.x/_line.controlVector.getLength) * 10),
             round(rotatePoint.y + (_line.controlVector.y/_line.controlVector.getLength) * 10));
    C.LineTo(round(rotatePoint.x + (_line.controlVector.x/_line.controlVector.getLength) * 60),
             round(rotatePoint.y + (_line.controlVector.y/_line.controlVector.getLength) * 60));

    //riadiaci bod
    C.Pen.Style := psSolid;
    C.Ellipse(round(rotatePoint.x + (_line.controlVector.x/_line.controlVector.getLength) * 80) - 5,
              round(rotatePoint.y + (_line.controlVector.y/_line.controlVector.getLength) * 80) - 5,
              round(rotatePoint.x + (_line.controlVector.x/_line.controlVector.getLength) * 80) + 5,
              round(rotatePoint.y + (_line.controlVector.y/_line.controlVector.getLength) * 80) + 5);
  end;
end;

procedure TLine.drawMoveControls(C: TCanvas);
begin
  if (_line <> nil) then
  begin
    C.Pen.Mode := pmNot;
    ...
    C.Brush.Style := bsClear;
    C.MoveTo(_line.controlPoint.point.x + round(_line.controlVector.y *1000),
             _line.controlPoint.point.y - round(_line.controlVector.x*1000));
    C.LineTo(_line.controlPoint.point.x - round(_line.controlVector.y *1000),
             _line.controlPoint.point.y + round(_line.controlVector.x*1000));
  end;
end;

function TLine.getRotateControls(): TPoints;
begin
  SetLength(Result, 2);
  Result[0] := rotatePoint.point;
  Result[1].X := round(rotatePoint.x + (_line.controlVector.x/_line.controlVector.getLength) * 80);
  Result[1].Y := round(rotatePoint.y + (_line.controlVector.y/_line.controlVector.getLength) * 80);
end;

function TLine.toString(): string;
begin
  Result := _line.toString;
end;

function TLine.clickedOn(a: TPoint): boolean;
begin
  Result := distance(a) <= (width div 2);
end;

function TLine.clickedOn(x, y: Integer): boolean;
var
  a: TPoint;
begin
  a.X := x;
...
procedure TLine.drawCreatePreview(C: TCanvas; p: TPoint);
begin
  if (not created) then
  begin
    FreeAndNil(_line);
    if (_n = 0) then
      _line := TGeneralLineEquation.Create(p, p); //feature, ktora to spravi tak ako chcem, rovnobezne s osou X
    if (_n = 1) then
      _line := TGeneralLineEquation.Create(_point, p);
    draw(C);
...
procedure TLine.movePoint(i: Integer; p: TPoint);
begin
  if (i = 0) then
    move(p.X - _line.controlPoint.point.X, p.Y - _line.controlPoint.point.Y);
end;

function TLine.addPoint(a: TPoint; where: Integer=-1): boolean;
begin
  Result := false;
  if (not created) then
  begin
    ...
    begin
      _line := TGeneralLineEquation.Create(_point, a);
      _n := 2;
    end;
  end;
end;

function TLine.removePoint(i: Integer): boolean;
begin
  Result := false;
end;

function TLine.clone(): TShape;
begin
  Result := TLine.Create();
end;

procedure TLine.changeZoom(a: Real);
begin
  if (a > 0) then
  begin
    _point.X := round(_point.X*a/zoom);
    _point.Y := round(_point.Y*a/zoom);
    if (_line <> nil) then
    begin
      _line.controlPoint.x := _line.controlPoint.x*a/zoom;
      _line.controlPoint.y := _line.controlPoint.y*a/zoom;
    end;
    _zoom := a;
  end;
end;
...
begin
  Self.rotatePoint := TPrecisePoint.Create(NilPoint);
  _n := 0;
  _point := InfinityPoint;
  _line := nil;
  _zoom := 1;
  deactivate();
end;

constructor TLine.Create(a, b: TPoint; color: TColor = clBlack; width: Integer = 1);
begin
  Self.rotatePoint := TPrecisePoint.Create(b);
  _line := TGeneralLineEquation.Create(a, b);
  Self.color := color;
  Self.width := width;
  _n := 2;
  _zoom := 1;
...
constructor TLine.Create(x1, y1, x2, y2: Integer; color: TColor = clBlack; width: Integer = 1);
var
  a, b: TPoint;
begin
  a.X := x1;
...
begin
  ...
  color := col;
  Integer(buff) := Integer(buff) + SizeOf(TColor);
  System.Move(buff^, int, SizeOf(Integer));
  width := int;
  Integer(buff) := Integer(buff) + SizeOf(Integer);
  ...
  begin
    ...
    _points[i].x := float;
    Integer(buff) := Integer(buff) + SizeOf(Real);
    System.Move(buff^, float, SizeOf(Real));
    _points[i].y := float;
    Integer(buff) := Integer(buff) + SizeOf(Real);
  end;
...
begin
  ...
  Integer(p) := Integer(p) + SizeOf(Boolean);
  System.Move(_width, p^, SizeOf(Integer));
  Integer(p) := Integer(p) + SizeOf(Integer);
...
begin
  ...
  bgColorMode := bool;
  Integer(buff) := Integer(buff) + SizeOf(Boolean);
  System.Move(buff^, int, SizeOf(Integer));
  width := int;
  Integer(buff) := Integer(buff) + SizeOf(Integer);
  System.Move(buff^, float, SizeOf(Real));
